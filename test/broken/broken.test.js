...
describe('Error test: swig-loader', function() {
	...
	it('should throw nicely formatted error', function(done) {
		load(path.resolve(__dirname, 'template.js'), function(err, mod) {
			expect(err).to.exist;
			expect(err.message).to.contain('Could not resolve swig template. Cause: Error: Unexpected tag "xyz asd" on line 1 in file ');
			expect(err.message).to.contain('test/broken/template.html.');
